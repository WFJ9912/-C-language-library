...
#include<stdio.h>
int a[10], book[10], n;//此处特别说明一下；C语言的全局变量在没有赋值以前默认为0，因此这里的book数组无需全部再次赋值初始值0

//深度优先搜索-不撞南墙不回头
//问题：输入一个数n,输出1-n的全排列。
//如123的全排列是123,132,213,231,312,321
//
//数组a表示小盒子，变量step表示当前正处在第step个小盒子面前。a[step]=i就是将i号扑克牌放入到第step个盒子中。数组book来标记哪些牌已经使用
...
void dfs(int step)//step表示现在站在第几个盒子面前
{
	...
	for (i = 1; i <= n; i++)
	{
		...
		if (book[i] == 0)//book[i]等于0表示i号扑克牌在手上
		{
			...
			book[i] = 0;//这是非常重要的一步，一定要将刚刚尝试的扑克牌收回，才能进行下一次尝试
